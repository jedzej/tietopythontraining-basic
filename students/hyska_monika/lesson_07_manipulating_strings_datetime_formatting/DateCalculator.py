...
# function to add years, months, days, hours, minutes to date
...
# function to add years, days, hours, minutes to date
def add_to_date2(date, years, days, hours, minutes):
    new_date = date + datetime.timedelta(days=days,
                                         hours=hours,
                                         minutes=minutes)
    new_date2 = new_date.replace(new_date.year + years)
    print('Changed date: ', new_date2.strftime("%Y-%m-%d %H:%M"))
    return new_date2
...
add_to_date2(test_date, 2, 50, 5, 30)
add_to_date2(test_date, 20, 389, 15897, 65)
