...
def password_check(password):
    ...
    password_status = {'Password is strong.' : password_ok, 'length to short' : length_error,
                       'missing digits' : digit_error, 'missing uppercase' : uppercase_error,
                       'missing lowercase' : lowercase_error}
    result = [k for k, v in password_status.items() if v == True]
    print(', '.join(result))
    return password_ok
