...
def check_arg(args=None):
    ...
    parser.add_argument('-p', '--prefix', action='store', dest='file_prefix',
                        help='Searched prefix in files name',
                        default='spam')
    parser.add_argument('-g', '--gap_place', dest='gap_place',
                        help='Place in numerate when will be gap',
                        default=None)
    results = parser.parse_args(args)
    return results.file_prefix, results.gap_place
...
def run_filling_in_the_gaps():
    prefix, place_of_gap = check_arg(sys.argv[1:])
