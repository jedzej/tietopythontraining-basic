...
def main():
    ...
    for i in range(files):
        list_of_files = \
            input("input file and access rights separate by spaces: ").split()
        if len(list_of_files) >= 2:
            file_name = list_of_files[0]
            file_access_rihts = list_of_files[1:]
            files_with_access_rights.update({file_name: file_access_rihts})
        else:
            print('Probably you don\'t give access rights')
    operations = 0
    try:
    ...
    for k in range(operations):
        operation, file_name = \
            input("enter operation and file name separate by space").split()
